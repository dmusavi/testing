...
prepare_cgroup_filesystem() {
    ...
    # Check if cgroup2 is mounted, else mount it
    if ! mountpoint -q /sys/fs/cgroup; then
    ...
    # Modify config.json cgroupsPath if it exists
...
main() {
    ...
    prepare_cgroup_filesystem   # Prepare the cgroup filesystem and update config.json
